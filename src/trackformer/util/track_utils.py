...
def plot_frame(tracks, cmap, frame_id, frame_data, output_dir):
...
def get_cmap():
    cmap = rand_cmap(
        999999, type="bright", first_color_black=False, last_color_black=False
    )
    return cmap


def plot_sequence(
    tracks, data_loader, output_dir, write_images, generate_attention_maps
):
    ...
    for i, track in tracks.items():
        ...
        if frames:
            ...
            for f in range(min(frames), max(frames) + 1):
                bbox = np.array([x0_inter(f), y0_inter(f), x1_inter(f), y1_inter(f)])
                tracks[i][f]["bbox"] = bbox
        else:
            tracks[i][frames[0]]["bbox"] = np.array([x0[0], y0[0], x1[0], y1[0]])
